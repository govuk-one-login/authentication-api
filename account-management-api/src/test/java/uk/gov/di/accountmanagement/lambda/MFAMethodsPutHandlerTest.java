...
class MFAMethodsPutHandlerTest {
    ...
    private static final UserProfile userProfile =
            new UserProfile().withSubjectID(TEST_INTERNAL_SUBJECT.getValue()).withEmail(EMAIL);
    ...
    void shouldReturn400WhenJsonIsInvalid() {
        var event = generateApiGatewayEvent(internalSubject).withBody("Invalid JSON");
        when(authenticationService.getOptionalUserProfileFromPublicSubject(PUBLIC_SUBJECT_ID))
                .thenReturn(Optional.of(userProfile));
    ...
    void shouldReturn400WhenPathParameterIsEmpty() {
        var event =
                generateApiGatewayEvent(internalSubject)
                        .withPathParameters(
                                Map.of(
                                        "mfaIdentifier",
                                        "some-mfa-identifier",
    ...
    void shouldReturn404WhenPrincipalIsInvalid() {
        when(authenticationService.getOptionalUserProfileFromPublicSubject(TEST_PUBLIC_SUBJECT_ID))
                .thenReturn(Optional.of(userProfile));
