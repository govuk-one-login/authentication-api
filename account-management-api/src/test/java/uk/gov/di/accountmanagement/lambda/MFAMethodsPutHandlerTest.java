...
class MFAMethodsPutHandlerTest {
        ...
        if (migrationFailureReason == MfaMigrationFailureReason.ALREADY_MIGRATED) {
            verify(auditService)
                    .submitAuditEvent(
                            ...
                            eq(pair("journey-type", ACCOUNT_MANAGEMENT.getValue())),
                            eq(pair("mfa-method", DEFAULT.name().toLowerCase())));
        } else {
    ...
    void shouldRaiseSwitchFailedAuditEvent() {
        ...
        var expectedAuditContext = BASE_AUDIT_CONTEXT.withPhoneNumber(UK_MOBILE_NUMBER);
