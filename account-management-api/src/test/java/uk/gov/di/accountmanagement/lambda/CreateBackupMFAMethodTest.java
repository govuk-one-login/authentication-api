...
class CreateBackupMFAMethodTest {
    ...
    void setUp() {
        ...
        handler = new CreateBackupMFAMethod(configurationService, dynamoService);
    }

    @Test
    void shouldReturn200WhenAndHelloWorld() {
        when(dynamoService.getUserProfileFromPublicSubject("helloPath")).thenReturn(USER_PROFILE);
        when(USER_PROFILE.getEmail()).thenReturn(EMAIL);
