...
public class MFAMethodsCreateHandler
    ...
    private APIGatewayProxyResponseEvent mfaMethodsHandler(
        ...
        if (addBackupMfaResult.isFailure()) {
            var maybeAuditContext = updateAuditContextForFailedMFACreation(userProfile, auditContext);
            if (maybeAuditContext.isFailure()) {
                return generateApiGatewayProxyErrorResponse(500, maybeAuditContext.getFailure());
            }
            auditService.submitAuditEvent(AUTH_MFA_METHOD_ADD_FAILED, maybeAuditContext.getSuccess());
        ...
        if (mfaMethodCreateRequest.mfaMethod().method() instanceof RequestSmsMfaDetail requestSmsMfaDetail) {
            auditContext =
                    auditContext.withPhoneNumber(requestSmsMfaDetail.phoneNumber());
        }
    ...
    private Result<ErrorResponse, AuditContext> updateAuditContextForFailedMFACreation(UserProfile userProfile, AuditContext auditContext) {
        ...
        var defaultMfaMethod = mfaMethods.stream()
                .filter(method -> method.getPriority().equalsIgnoreCase(PriorityIdentifier.DEFAULT.name()))
                .findFirst();

        if (defaultMfaMethod.get().getMfaMethodType().equalsIgnoreCase(MFAMethodType.SMS.name())) {
            auditContext =
                    auditContext.withPhoneNumber(defaultMfaMethod.get().getDestination());
        }
