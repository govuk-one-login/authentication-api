...
public class OrchestrationAuthorizationService {
    ...
    public Optional<AuthRequestError> validateAuthRequest(
            AuthenticationRequest authRequest, boolean isNonceRequired) throws ParseException {

        ClientRegistry client = getClientFromDynamo(authRequest.getClientID().toString());
        if (isJarRequired(authRequest)) {
            if (authRequest.getRequestObject() == null) {
                logAndThrowRuntimeException(
                        "JAR required but request does not contain Request Object");
            }
            var redirectUri = getOrGenerateRedirectUriFromRequestObject(authRequest, client);
            var errorResponse = validateClientType(client, redirectUri);
            if (errorResponse.isPresent()) {
                return errorResponse;
            }
            LOG.info("Validating request using JAR");
            return validateRequestUsingJar(authRequest, redirectUri, client);
        } else {
            LOG.info("Validating request query params");
            return validateQueryParams(authRequest, client, isNonceRequired);
        }
    }

    private Optional<AuthRequestError> validateQueryParams(
            AuthenticationRequest authRequest, ClientRegistry client, boolean isNonceRequired) {
    ...
    private URI getOrGenerateRedirectUriFromRequestObject(
            AuthenticationRequest authRequest, ClientRegistry client) throws ParseException {
        ...
        if (jwtClaimsSet.getStringClaim("redirect_uri") == null
            ...
            logAndThrowRuntimeException("Invalid Redirect URI in request JWT");
        }

        return URI.create((String) jwtClaimsSet.getClaim("redirect_uri"));
    }
    ...
    private Optional<AuthRequestError> validateClientType(ClientRegistry client, URI redirectURI) {
    ...
    private boolean isJarRequired(AuthenticationRequest authRequest) {
        return authRequest.getScope().contains(CustomScopeValue.DOC_CHECKING_APP);
    }

    private Optional<AuthRequestError> validateRequestUsingJar(
            AuthenticationRequest authRequest, URI redirectUri, ClientRegistry client) {

        Optional<AuthRequestError> authRequestError;
        authRequestError = validateJarParameters(authRequest, redirectUri, client);
        if (!authRequestError.isPresent()) {
            authRequestError = validateRequestObject(authRequest, redirectUri, client);
        }
        return authRequestError;
    }

    private Optional<AuthRequestError> validateJarParameters(
            AuthenticationRequest authRequest, URI redirectURI, ClientRegistry client) {
    ...
    private Optional<AuthRequestError> validateRequestObject(
            AuthenticationRequest authRequest, URI redirectURI, ClientRegistry client) {

        var signedJWT = (SignedJWT) authRequest.getRequestObject();
        var signatureValid = isSignatureValid(signedJWT, client.getPublicKey());
        if (!signatureValid) {
            logAndThrowRuntimeException("Invalid Signature on request JWT");
        }
        ...
        try {
            ...
            if (Objects.nonNull(jwtClaimsSet.getClaim("ui_locales"))) {
                var uiLocalesError =
                        validateUiLocales(
                                (String) jwtClaimsSet.getClaim("ui_locales"), redirectURI);
                if (uiLocalesError.isPresent()) {
                    return uiLocalesError;
                }
            }
            LOG.info("Request Object has passed initial validation");
            return Optional.empty();
        } catch (ParseException e) {
            LOG.warn("Validating Request Object failed");
    ...
    private Optional<AuthRequestError> validateUiLocales(String uiLocales, URI redirectURI) {
        try {
            ...
            return errorResponse(
                    redirectURI,
                    new ErrorObject(
                            OAuth2Error.INVALID_REQUEST_CODE, "ui_locales parameter is invalid"));
        }
        return Optional.empty();
    }
}
