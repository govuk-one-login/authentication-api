...
public class AccessTokenService {
    ...
    public AccessTokenInfo parse(String authorizationHeader, boolean identityEnabled)
        ...
        try {
            ...
            throw new AccessTokenException(
                    "Unable to get Orch Access Token from Dynamo", BearerTokenError.INVALID_TOKEN);
        }
        if (orchAccessTokenItem.isEmpty()) {
            LOG.warn("There is no access token in dynamo matching the token in the request");
        } else {
            LOG.info("Token value from dynamo matches the token value in redis");
            LOG.info(
                    "Does internal pairwise subject id from dynamo match redis? {}",
                    Objects.equals(
                            orchAccessTokenItem.get().getInternalPairwiseSubjectId(),
                            accessTokenStore.get().getInternalPairwiseSubjectId()));
            LOG.info(
                    "Does client session id from dynamo match redis? {}",
                    Objects.equals(
