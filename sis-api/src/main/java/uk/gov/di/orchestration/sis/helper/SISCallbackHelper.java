package uk.gov.di.orchestration.sis.helper;
...
import uk.gov.di.orchestration.sis.entity.LogIds;
import uk.gov.di.orchestration.sis.entity.SPOTClaims;
import uk.gov.di.orchestration.sis.entity.SPOTRequest;
import uk.gov.di.orchestration.sis.exception.SISCallbackException;
...
public class SISCallbackHelper {
    private static final Logger LOG = LogManager.getLogger(SISCallbackHelper.class);
    ...
    public SISCallbackHelper(ConfigurationService configurationService) {
        ...
        for (VectorOfTrust vtr : vtrList) {
            if (vtr.getLevelOfConfidence()
                ...
                if (!trustmarkURL.equals(userIdentityUserInfo.getClaim(VTM.getValue()))) {
                    ...
                    throw new SISCallbackException("IPV trustmark is invalid");
                }
