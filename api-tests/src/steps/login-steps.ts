...
import {
  ...
  StartResponse,
  startSession,
} from "../clients/frontend-api-client.js";
...
When(
  "I check that the user {string} exists",
...
Then(
  ...
  function (this: World, doesNot: " does not") {
    assert.equal(
      (this.lastResponse as CheckUserExistsResponse).doesUserExist,
      !doesNot,
    );
  },
);
...
Then(
  /the user( does not)? requires? MFA/,
  function (this: World, doesNot: " does not") {
