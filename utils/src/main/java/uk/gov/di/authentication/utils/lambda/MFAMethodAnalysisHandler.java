...
public class MFAMethodAnalysisHandler implements RequestHandler<String, Long> {
    ...
    public MFAMethodAnalysisHandler(
        ...
        userCredentialsTableName =
                format("{0}-user-credentials", this.configurationService.getEnvironment());
        userProfileTableName =
                format("{0}-user-profile", this.configurationService.getEnvironment());
    }
            ...
            do {
                ...
                for (Map<String, AttributeValue> userCredentialsItem : scanResponse.items()) {
                    ...
                    if (currentBatchEmails.size() >= 100) {
                        totalBatches++;
                        queueBatch(currentBatchEmails, totalBatches, batchTasks);
    ...
    private void gracefulPoolShutdown() {
        forkJoinPool.shutdown();
        try {
            if (!forkJoinPool.awaitTermination(Long.MAX_VALUE, TimeUnit.MILLISECONDS)) {
    ...
    private void forcePoolShutdown() {
        if (!forkJoinPool.isShutdown()) {
            forkJoinPool.shutdownNow();
        }
    }

    private void queueBatch(List<String> emails, long batch, List<ForkJoinTask<Long>> batchTasks) {
