...
class CheckReAuthUserHandlerTest {
    ...
    void shouldReturn404ForWhenUserDoesNotMatch(boolean supportAuthenticationAttempts) {
        ...
        when(clientRegistry.getRedirectUrls()).thenReturn(List.of(INTERNAL_SECTOR_URI));
        when(userContext.getTxmaAuditEncoded()).thenReturn(null);
        when(codeStorageService.getIncorrectEmailCount(EMAIL_USED_TO_SIGN_IN)).thenReturn(1);
        when(authenticationAttemptsService.getCount(
                        TEST_SUBJECT_ID, JourneyType.REAUTHENTICATION, CountType.ENTER_EMAIL))
                .thenReturn(1);
        ...
        verify(auditService)
                .submitAuditEvent(
                        ...
                        pair("user_supplied_email", EMAIL_USED_TO_SIGN_IN),
                        pair("user_id_for_user_supplied_email", TEST_SUBJECT_ID),
                        pair("rp_pairwise_id", expectedRpPairwiseSub));
