...
public class LogLineHelper {
    ...
    public static void updateAttachedOrchSessionIdToLogs(String orchSessionId) {
        if (ThreadContext.containsKey(ORCH_SESSION_ID.getLogFieldName())) {
            ThreadContext.remove(ORCH_SESSION_ID.getLogFieldName());
        }
        attachSessionIdToLogs(orchSessionId);
    }
}
