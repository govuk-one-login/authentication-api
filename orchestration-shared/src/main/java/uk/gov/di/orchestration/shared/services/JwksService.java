...
public class JwksService {
    ...
    private JWK createJwk(String keyId) {
        ...
        if (publicKeyResponse.signingAlgorithms().contains(ECDSA_SHA_256)) {
            var publicKey = createEcPublicKey(publicKeyResponse);
            ...
            var publicKey = createRsaPublicKey(publicKeyResponse);
    ...
    private PublicKey createEcPublicKey(GetPublicKeyResponse publicKeyResponse) {
        try {
            ...
            LOG.error("NoSuchAlgorithmException when creating EC public key", e);
            throw new RuntimeException();
        } catch (InvalidKeySpecException e) {
            LOG.error("InvalidKeySpecException when creating EC public key", e);
            throw new RuntimeException();
        }
    }

    private PublicKey createRsaPublicKey(GetPublicKeyResponse publicKeyResponse) {
        try {
            X509EncodedKeySpec keySpec =
                    new X509EncodedKeySpec(publicKeyResponse.publicKey().asByteArray());
            ...
            LOG.error("NoSuchAlgorithmException when creating RSA public key", e);
            throw new RuntimeException();
        } catch (InvalidKeySpecException e) {
