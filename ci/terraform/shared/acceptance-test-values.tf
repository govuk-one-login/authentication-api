...
resource "aws_ssm_parameter" "at_rp_url" {
  ...
  type  = "String"
  value = var.orch_stub_deployed ? "https://orchstub.signin.${local.service_domain}/" : "${var.stub_rp_clients[index(var.stub_rp_clients.*.at_client, true)].sector_identifier_uri}/?relyingParty=${random_string.stub_relying_party_client_id[var.stub_rp_clients[0].client_name].result}"
}
...
resource "aws_ssm_parameter" "at_doc_app_url" {
  count = contains([for client in var.stub_rp_clients : client.client_name], "relying-party-stub-${var.environment}-app") ? 1 : 0
