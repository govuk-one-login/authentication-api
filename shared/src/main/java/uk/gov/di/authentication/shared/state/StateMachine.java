...
import static uk.gov.di.authentication.shared.entity.SessionAction.USER_CHOSE_TO_REGISTER_NEW_USER;
import static uk.gov.di.authentication.shared.entity.SessionAction.USER_ENTERED_A_NEW_PHONE_NUMBER;
import static uk.gov.di.authentication.shared.entity.SessionAction.USER_ENTERED_CORRECT_PHONE_VERIFICATION_CODE;
import static uk.gov.di.authentication.shared.entity.SessionAction.USER_ENTERED_INCORRECT_PHONE_VERIFICATION_CODE;
import static uk.gov.di.authentication.shared.entity.SessionAction.USER_ENTERED_INCORRECT_PHONE_VERIFICATION_CODE_TOO_MANY_TIMES;
...
import static uk.gov.di.authentication.shared.entity.SessionAction.USER_ENTERED_VALID_PASSWORD;
import static uk.gov.di.authentication.shared.entity.SessionAction.USER_HAS_VALIDATED_EMAIL;
...
public class StateMachine<T, A, C> {
    ...
    public static StateMachine<SessionState, SessionAction, UserProfile> userJourneyStateMachine() {
        Map<SessionState, List<Transition<SessionState, SessionAction, UserProfile>>> states =
                ofEntries(
                        entry(NEW, List.of(new Transition<>(USER_CHOSE_TO_REGISTER_NEW_USER, USER_NOT_FOUND))),
                        entry(
                                USER_NOT_FOUND,
                                List.of(
                                        new Transition<>(USER_ENTERED_UNREGISTERED_EMAIL_ADDRESS, USER_NOT_FOUND),
                                        new Transition<>(USER_ENTERED_REGISTERED_EMAIL_ADDRESS, AUTHENTICATION_REQUIRED),
                                        new Transition<>(USER_ENTERED_VALID_PASSWORD, VERIFY_EMAIL_CODE_SENT))),
                        entry(
                                VERIFY_EMAIL_CODE_SENT,
                                List.of(
                                        new Transition<>(USER_ENTERED_VALID_EMAIL_VERIFICATION_CODE, EMAIL_CODE_VERIFIED),
                                        new Transition<>(USER_ENTERED_INVALID_EMAIL_VERIFICATION_CODE, EMAIL_CODE_NOT_VALID))),
                        entry(
                                EMAIL_CODE_NOT_VALID,
                                List.of(
                                        new Transition<>(USER_ENTERED_VALID_EMAIL_VERIFICATION_CODE, EMAIL_CODE_VERIFIED),
                                        new Transition<>(USER_ENTERED_INVALID_EMAIL_VERIFICATION_CODE, EMAIL_CODE_NOT_VALID),
                                        new Transition<>(USER_ENTERED_INVALID_EMAIL_VERIFICATION_CODE_TOO_MANY_TIMES, EMAIL_CODE_MAX_RETRIES_REACHED))),
                        entry(EMAIL_CODE_MAX_RETRIES_REACHED, Collections.emptyList()),
                        entry(
                                EMAIL_CODE_VERIFIED,
                                List.of(
                                        new Transition<>(USER_HAS_VALIDATED_EMAIL, TWO_FACTOR_REQUIRED))),
                        entry(
                                TWO_FACTOR_REQUIRED,
                                List.of(
                                        new Transition<>(USER_ENTERED_A_NEW_PHONE_NUMBER, ADDED_UNVERIFIED_PHONE_NUMBER))),
                        entry(
                                ADDED_UNVERIFIED_PHONE_NUMBER,
                                List.of(
                                        new Transition<>(SYSTEM_HAS_SENT_PHONE_VERIFICATION_CODE, VERIFY_PHONE_NUMBER_CODE_SENT))),
                        entry(
                                VERIFY_PHONE_NUMBER_CODE_SENT,
                                List.of(
                                        new Transition<>(USER_ENTERED_CORRECT_PHONE_VERIFICATION_CODE, PHONE_NUMBER_CODE_VERIFIED),
                                        new Transition<>(USER_ENTERED_INCORRECT_PHONE_VERIFICATION_CODE, PHONE_NUMBER_CODE_NOT_VALID))),
                        entry(PHONE_NUMBER_CODE_VERIFIED,
                                List.of(
                                        new Transition<>(SYSTEM_HAS_ISSUED_AUTHORIZATION_CODE, AUTHENTICATED))),
                        entry(
                                PHONE_NUMBER_CODE_NOT_VALID,
                                List.of(
                                        new Transition<>(USER_ENTERED_CORRECT_PHONE_VERIFICATION_CODE, PHONE_NUMBER_CODE_VERIFIED),
                                        new Transition<>(USER_ENTERED_INCORRECT_PHONE_VERIFICATION_CODE, PHONE_NUMBER_CODE_NOT_VALID),
                                        new Transition<>(USER_ENTERED_INCORRECT_PHONE_VERIFICATION_CODE_TOO_MANY_TIMES, PHONE_NUMBER_CODE_MAX_RETRIES_REACHED))),
                        ...
                        entry(AUTHENTICATION_REQUIRED,
                                List.of(
                                        ...
                                        new Transition<>(USER_ENTERED_UNREGISTERED_EMAIL_ADDRESS, USER_NOT_FOUND))),
                        entry(LOGGED_IN, List.of(
                                new Transition<>(SYSTEM_HAS_SENT_MFA_CODE, MFA_SMS_CODE_SENT))),
                        entry(
                                MFA_SMS_CODE_SENT,
                                List.of(
                                        new Transition<>(SYSTEM_HAS_SENT_MFA_CODE, MFA_SMS_CODE_SENT),
                                        new Transition<>(USER_ENTERED_VALID_MFA_CODE, MFA_CODE_VERIFIED),
                                        new Transition<>(USER_ENTERED_INVALID_MFA_CODE, MFA_CODE_NOT_VALID))),
                        entry(
                                MFA_CODE_NOT_VALID,
                                List.of(
                                        new Transition<>(USER_ENTERED_VALID_MFA_CODE, MFA_CODE_VERIFIED),
                                        new Transition<>(USER_ENTERED_INVALID_MFA_CODE, MFA_CODE_NOT_VALID),
                                        new Transition<>(USER_ENTERED_INVALID_MFA_CODE_TOO_MANY_TIMES, MFA_CODE_MAX_RETRIES_REACHED))),
                        entry(MFA_CODE_MAX_RETRIES_REACHED, Collections.emptyList()),
                        entry(MFA_CODE_VERIFIED, List.of(
                                new Transition<>(SYSTEM_HAS_ISSUED_AUTHORIZATION_CODE, AUTHENTICATED))),
                        entry(AUTHENTICATED,
                                List.of(
                                        new Transition<>(SYSTEM_HAS_ISSUED_AUTHORIZATION_CODE, AUTHENTICATED))));
